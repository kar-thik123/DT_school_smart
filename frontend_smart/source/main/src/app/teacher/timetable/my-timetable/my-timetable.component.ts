...
import { MyTimetableFormComponent } from './dialogs/form-dialog/form-dialog.component';
import { MyTimetableDeleteComponent } from './dialogs/delete/delete.component';
...
export class MyTimetableComponent implements OnInit, OnDestroy {
  ...
  dataSource = new MatTableDataSource<TimetableEntry>([]);
  isLoading = true;
  ...
  handleEdit(row: TimetableEntry) {
    this.openDialog('edit', row);
  }

  openDialog(action: 'add' | 'edit', data?: TimetableEntry) {
    const varDirection: Direction =
      this.localStorageService.get('isRtl') === 'true' ? 'rtl' : 'ltr';
    const dialogRef = this.dialog.open(MyTimetableFormComponent, {
      width: '60vw',
      maxWidth: '100vw',
      data: { timetable: data, action },
  ...
  private updateRecord(updatedRecord: TimetableEntry) {
  ...
  handleDelete(row: TimetableEntry) {
    const dialogRef = this.dialog.open(MyTimetableDeleteComponent, {
      data: row,
    });
    dialogRef.afterClosed().subscribe((result) => {
      if (result) {
  ...
  handleBulkDelete(selectedRows: TimetableEntry[]) {
