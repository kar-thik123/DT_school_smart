...
import { AcademicStructureService, IGrade, ISection, ISubjectGroup } from '../units-list/services/units.service';
...
export class StaffAttendanceComponent implements OnInit, OnDestroy {
  ...
  attendanceService = inject(StaffAttendanceService);
  ...
  ngOnInit() {
    this.hasManagePermission = this.authService.hasPermission('STAFF_ATTENDANCE', 'MANAGE');
  ...
  handleRefresh() {
    ...
    let enrollUrl = `${environment.apiUrl}/staff-attendance/staff-list`;
    ...
    this.httpClient.get<any[]>(enrollUrl).subscribe({
      next: (enrollments) => {
        if (!this.morningPhaseId || !this.afternoonPhaseId) {
          const data = enrollments.map((e, index) => ({
            ...
            staff_id: e.staff_id,
            name: e.staff.name,
            roll_number: e.staff.roll_number,
        ...
        forkJoin({
          morning: this.attendanceService.getDailyAttendance(this.morningPhaseId, dateStr),
          afternoon: this.attendanceService.getDailyAttendance(this.afternoonPhaseId, dateStr)
        }).subscribe({
          next: ({ morning, afternoon }) => {
            const morningMap = new Map();
            morning.forEach((r: any) => morningMap.set(r.staff_id, r));

            const afternoonMap = new Map();
            afternoon.forEach((r: any) => afternoonMap.set(r.staff_id, r));

            const data = enrollments.map((e, index) => {
              const mExisting = morningMap.get(e.staff_id);
              const aExisting = afternoonMap.get(e.staff_id);
      ...
      error: () => {
        this.isLoading = false;
        this.showNotification('Error loading staff');
      }
    });
  }
      ...
      if (differences.length > 0) {
        let diffHtml = `
          <p style="font-size: 15px; color: #4b5563; margin-top: 10px; margin-bottom: 15px; text-align: center; line-height: 1.5;">
            <strong class="text-danger" style="font-size: 18px;">${differences.length}</strong> staff(s) have different attendance statuses between Morning and Afternoon.
          </p>
  ...
  copyMorningForSelected() {
    ...
    if (changed) {
      this.updateAfternoonVisibility();
      this.dataSource._updateChangeSubscription();
      this.showNotification('Morning status copied for selected staff.');
    }
  }
  ...
  markSelected(status: string) {
    ...
    if (changed) {
      this.updateAfternoonVisibility();
      this.dataSource._updateChangeSubscription();
      this.showNotification(`Selected staff marked as ${status}.`);
    } else {
      this.showNotification('No staff selected.');
    }
  }
  ...
  markRest(status: 'PRESENT' | 'ABSENT') {
    ...
    if (changed) {
      this.updateAfternoonVisibility();
      this.dataSource._updateChangeSubscription();
      this.showNotification(`Unmarked staff have been set to ${status}.`);
    } else {
      this.showNotification('All staff are already marked.');
    }
  }
  ...
  loadRangeAttendance() {
    ...
    forkJoin({
      ...
      attendance: this.attendanceService.getRangeAttendance(startStr, endStr)
    }).subscribe({
      next: ({ enrollments, attendance }) => {
        ...
        enrollments.forEach(e => {
          staffMap.set(e.staff_id, {
            name: e.staff.name,
            avatar: e.staff.user_profile?.profile_image || 'assets/images/user/user1.jpg',
          ...
          if (staffMap.has(record.staff_id)) {
            ...
            if (dayIndex >= 0 && dayIndex < daysCount) {
              ...
              if (phaseId === this.morningPhaseId || phaseName === 'morning') {
                staff.morningStatus[dayIndex] = record.status;
              } else if (phaseId === this.afternoonPhaseId || phaseName === 'afternoon') {
                staff.afternoonStatus[dayIndex] = record.status;
              } else {
                // Fallback: Try to populate morning first, then afternoon
                if (staff.morningStatus[dayIndex] === '-') {
                  staff.morningStatus[dayIndex] = record.status;
                } else if (staff.afternoonStatus[dayIndex] === '-') {
                  staff.afternoonStatus[dayIndex] = record.status;
                }
              }
            }
        ...
        staffMap.forEach(staff => {
          for (let i = 0; i < daysCount; i++) {
            const m = staff.morningStatus[i];
            const a = staff.afternoonStatus[i];
            ...
            if (isMPresent && isAPresent) {
              staff.attendanceStatus[i] = 'present';
            } else if (isMAbsent && isAAbsent) {
              staff.attendanceStatus[i] = 'leave';
            } else if (isMPresent && isAAbsent) {
              // Afternoon leave
              staff.attendanceStatus[i] = 'half_day_afternoon';
            } else if (isMAbsent && isAPresent) {
              // Morning leave
              staff.attendanceStatus[i] = 'half_day_morning';
        ...
        for (let i = 0; i < daysCount; i++) {
          ...
          if (dayOfWeek === 0 || dayOfWeek === 6) {
            staffMap.forEach(staff => {
              if (staff.attendanceStatus[i] === '-') {
                staff.attendanceStatus[i] = 'weekend';
              }
            });
          }
        ...
        this.rangeAttendanceData = Array.from(staffMap.values());
        this.isLoading = false;
      },
      error: () => {
    ...
    if (formValues) {
      ...
      if (morningChanged && newMorning) {
        requests.push(this.attendanceService.markAttendance({
          ...
          phase_id: this.morningPhaseId,
          attendance_date: dateStr,
          records: [{ staff_id: row.staff_id, status: newMorning as any }]
        }));
      }
      ...
      if (afternoonChanged && newAfternoon) {
        requests.push(this.attendanceService.markAttendance({
          ...
          phase_id: this.afternoonPhaseId,
          attendance_date: dateStr,
          records: [{ staff_id: row.staff_id, status: newAfternoon as any }]
        }));
      }
