...
import { RoutesFormComponent } from './dialogs/form-dialog/form-dialog.component';
import { RoutesDeleteComponent } from './dialogs/delete/delete.component';
...
export class RoutesPageComponent implements OnInit, OnDestroy {
  ...
  columnDefinitions: ColumnDefinition[] = [
    ...
    { def: 'route_name', label: 'Route Name', type: 'text', visible: true },
    { def: 'start_point', label: 'Start Point', type: 'text', visible: true },
    { def: 'end_point', label: 'End Point', type: 'text', visible: true },
    { def: 'distance', label: 'Distance', type: 'text', visible: true },
    { def: 'vehicle_no', label: 'Vehicle No', type: 'text', visible: true },
    { def: 'route_fees', label: 'Fees', type: 'text', visible: true },
    {
      def: 'status',
      label: 'Status',
      type: 'status',
  ...
  handleEdit(row: TransportRoute) {
    this.openDialog('edit', row);
  }

  openDialog(action: 'add' | 'edit', data?: TransportRoute) {
    ...
    const dialogRef = this.dialog.open(RoutesFormComponent, {
      ...
      maxWidth: '100vw',
      data: {
        route: data ? data : new TransportRoute({} as TransportRoute),
        action: action,
      },
  ...
  handleDelete(row: TransportRoute) {
    const varDirection: Direction =
      this.localStorageService.get('isRtl') === 'true' ? 'rtl' : 'ltr';
    const dialogRef = this.dialog.open(RoutesDeleteComponent, {
      data: row,
      direction: varDirection,
    });
    dialogRef.afterClosed().subscribe((result) => {
