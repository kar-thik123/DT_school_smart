...
gradeRouter.get('/', (0, auth_middleware_1.requirePermission)('ACADEMIC_STRUCTURE', 'READ'), async (req, res) => {
    try {
        ...
        let filter = { organization_id: req.user.organization_id, academic_year_id: yearId };
        if (!isGlobalAdmin) {
...
router.post('/sections', (0, auth_middleware_1.requirePermission)('ACADEMIC_STRUCTURE', 'CREATE'), async (req, res) => {
    try {
        ...
        const parentGrade = await prisma_1.default.grade.findFirst({
        ...
        if (sort_order !== undefined)
            cleanData.sort_order = Number(sort_order);
        if (is_active !== undefined)
            cleanData.is_active = is_active === true || is_active === 'true';
        if (capacity !== undefined)
            cleanData.capacity = capacity === null || capacity === '' ? null : Number(capacity);
        if (warning_threshold !== undefined)
            cleanData.warning_threshold = warning_threshold === null || warning_threshold === '' ? null : Number(warning_threshold);
        const data = await prisma_1.default.section.create({
            data: cleanData
        });
...
router.put('/sections/:id', (0, auth_middleware_1.requirePermission)('ACADEMIC_STRUCTURE', 'EDIT'), async (req, res) => {
    try {
        ...
        if (warning_threshold !== undefined)
            cleanData.warning_threshold = warning_threshold === null || warning_threshold === '' ? null : Number(warning_threshold);
        const data = await prisma_1.default.section.update({
            where: { id: req.params.id },
            data: cleanData
        });
