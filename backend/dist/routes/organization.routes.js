...
router.post('/upload-school-profile', auth_middleware_1.authMiddleware, (req, res, next) => {
    const userPermissions = req.user.permissions || [];
    const isSuperAdmin = authorization_service_1.AuthorizationService.hasIdentity(userPermissions, 'IS_SUPER_ADMIN');
    const isSystemAdmin = authorization_service_1.AuthorizationService.hasIdentity(userPermissions, 'IS_SYSTEM_ADMIN');
    if (!isSuperAdmin && !isSystemAdmin) {
    ...
    next();
}, uploadProfile.single('profile'), (req, res) => {
    if (!req.file)
        return res.status(400).json({ message: 'No file uploaded' });
...
router.patch('/:id/settings', auth_middleware_1.authMiddleware, (0, auth_middleware_1.requirePermission)('IDENTITY', 'IS_SYSTEM_ADMIN'), async (req, res) => {
    ...
    try {
        ...
        const { school_name, contact_email, logo_url, profile_url, subdomain, domain_type, login_limit, smtp_host, smtp_port, backup_enabled, admin_email, admin_password, initial_academic_year } = req.body;
        ...
        let newPasswordHash;
        if (admin_password) {
            newPasswordHash = await bcrypt_1.default.hash(admin_password, 10);
        }
        // Check if new admin_email is already taken globally
        if (admin_email) {
            const existingUser = await prisma_1.default.user.findFirst({ where: { email: admin_email } });
            ...
            if (admin_email || newPasswordHash) {
                ...
                if (superAdminRole) {
                    ...
                    if (adminUser) {
                        ...
                        if (admin_email)
                            userUpdateData.email = admin_email;
                        if (newPasswordHash)
                            userUpdateData.password_hash = newPasswordHash;
...
const orgSchema = zod_1.z.object({
    ...
    profile_url: zod_1.z.string().nullish().or(zod_1.z.literal('')),
    // Domain
...
// ==========================================
// 1. Save New Draft
router.post('/draft', auth_middleware_1.authMiddleware, (0, auth_middleware_1.requirePermission)('IDENTITY', 'IS_SYSTEM_ADMIN'), async (req, res) => {
    try {
        ...
        if (cleanedBody.subdomain) {
            const existing = await prisma_1.default.organization.findUnique({ where: { subdomain: cleanedBody.subdomain } });
            if (existing) {
...
router.patch('/draft/:id', auth_middleware_1.authMiddleware, (0, auth_middleware_1.requirePermission)('IDENTITY', 'IS_SYSTEM_ADMIN'), async (req, res) => {
    try {
        const orgId = req.params.id;
        const cleanedBody = cleanInput(req.body);
        const currentOrg = await prisma_1.default.organization.findUnique({ where: { id: orgId }, include: { license: true } });
        if (!currentOrg)
            return res.status(404).json({ message: 'Draft not found' });
        if (currentOrg.status !== 'DRAFT')
            return res.status(400).json({ message: 'Cannot edit a non-draft organization here' });
...
router.get('/draft/:id', auth_middleware_1.authMiddleware, (0, auth_middleware_1.requirePermission)('IDENTITY', 'IS_SYSTEM_ADMIN'), async (req, res) => {
    try {
        const org = await prisma_1.default.organization.findUnique({
            where: { id: req.params.id },
            include: { license: true }
        ...
        if (!org)
            return res.status(404).json({ message: 'Draft not found' });
        if (org.status !== 'DRAFT')
            return res.status(400).json({ message: 'Organization is not a draft' });
...
// 4. Delete Draft
router.delete('/draft/:id', auth_middleware_1.authMiddleware, (0, auth_middleware_1.requirePermission)('IDENTITY', 'IS_SYSTEM_ADMIN'), async (req, res) => {
    try {
        const org = await prisma_1.default.organization.findUnique({ where: { id: req.params.id } });
        if (!org)
            return res.status(404).json({ message: 'Draft not found' });
        if (org.status !== 'DRAFT')
            return res.status(400).json({ message: 'Cannot delete a non-draft organization' });
        if (org.logo_url)
            deleteLogoFileSafely(org.logo_url);
        if (org.profile_url)
            deleteLogoFileSafely(org.profile_url);
        await prisma_1.default.organization.delete({ where: { id: org.id } });
