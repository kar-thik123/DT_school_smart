    ...
    try {
        ...
        const mappedLogs = logs.map((log) => {
            ...
            if (log.action_type === 'DELETE')
                severity = 'Warning';
            else if (log.action_type === 'LOGIN_FAILURE')
                severity = 'Critical';
            else if (log.action_type === 'ROLE_CHANGE' || log.action_type === 'PERMISSION_SYNC' || log.action_type === 'ACCESS_REVOKED')
                severity = 'Alert';
            return {
                id: log.id,
...
router.put('/:module_name', async (req, res) => {
    try {
        const { module_name } = req.params;
        ...
        if (module_name === 'completion') {
            if (typeof config_data.enable_module === 'boolean')
                sanitizedConfig['enable_module'] = config_data.enable_module;
            if (typeof config_data.enable_notifications === 'boolean')
                sanitizedConfig['enable_notifications'] = config_data.enable_notifications;
        }
        else if (module_name === 'mcq') {
            if (typeof config_data.enable_module === 'boolean')
                sanitizedConfig['enable_module'] = config_data.enable_module;
        ...
        const config = await prisma_1.default.moduleConfig.upsert({
            where: {
                organization_id_module_name: {
                    organization_id: org_id,
                    module_name
