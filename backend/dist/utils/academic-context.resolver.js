...
exports.AcademicContextResolver = void 0;
const academic_helper_1 = require("./academic-helper");
class AcademicContextResolver {
     ...
     */
    static async resolveAcademicYearId(req) {
        const orgId = req.user?.organization_id;
        if (!orgId)
            throw new Error('Organization ID is missing from user context');
        ...
        return await (0, academic_helper_1.getActiveAcademicYearId)(orgId);
    }
    /**
     ...
     */
    static async resolveHistoricalAcademicYearId(req) {
        const orgId = req.user?.organization_id;
        if (!orgId)
            throw new Error('Organization ID is missing from user context');
        let yearId = req.headers['x-academic-year-id'];
        if (!yearId) {
            // Fallback to query string for backward compatibility during rollout
            yearId = req.query?.academic_year_id;
            if (yearId) {
        ...
        if (!yearId || yearId === 'null' || yearId === 'undefined') {
            ...
            yearId = await (0, academic_helper_1.getActiveAcademicYearId)(orgId);
        }
        return yearId;
    }
    /**
     ...
     */
    static resolveMasterContext(req) {
        const orgId = req.user?.organization_id;
        if (!orgId)
            throw new Error('Organization ID is missing from user context');
...
exports.AcademicContextResolver = AcademicContextResolver;
