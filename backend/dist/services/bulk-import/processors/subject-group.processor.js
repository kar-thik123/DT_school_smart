...
exports.SubjectGroupProcessor = void 0;
...
class SubjectGroupProcessor {
    ...
    async resolveRelations(rows) {
        ...
        const sectionNames = Array.from(new Set(rows.map((r) => r.section_name?.trim().toLowerCase()).filter(Boolean)));
        // Parse all subject names in one pass
        const subjectNames = Array.from(new Set(rows.flatMap(r => (r.subjects || '').split(',').map((s) => s.trim().toLowerCase()).filter(Boolean))));
        ...
        if (gradeIds.length > 0) {
            ...
            if (sectionIds.length > 0) {
                allowedSectionSubjects = await prisma_1.default.subjectGroupSubject.findMany({
        ...
        for (const item of allowedSectionSubjects) {
            ...
            if (!this.resolved.sectionAllowedSubjects[secId]) {
                this.resolved.sectionAllowedSubjects[secId] = new Set();
            }
    ...
    async validateRow(row) {
        ...
        const subjectNames = rawSubjects.split(',').map((s) => s.trim().toLowerCase()).filter(Boolean);
        ...
        return {
            ...
            data: {
                ...
                resolved_subject_ids: resolvedSubjects.map((s) => s.id)
            }
        };
    }
...
exports.SubjectGroupProcessor = SubjectGroupProcessor;
