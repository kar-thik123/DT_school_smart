...
router.get('/management/overview', requirePermission('IDENTITY', 'IS_MANAGEMENT'), async (req: any, res: Response) => {
  try {
    ...
    // Overall Pass Rate
    const examResults = await prisma.studentExamResult.groupBy({
      by: ['result_status'],
    ...
    res.json({
      ...
      active_modules: await prisma.practiceAttempt.count({ where: { organization_id: org_id, academic_year_id: yearId } }),
      critical_alerts: 0
    });
  } catch (error) {
    console.error(error);
...
router.get('/management/curriculum-coverage', requirePermission('IDENTITY', 'IS_MANAGEMENT'), async (req: any, res: Response) => {
  try {
    const org_id = req.user.organization_id;
    const yearId = await AcademicContextResolver.resolveHistoricalAcademicYearId(req);
    const grade_id = req.query.grade_id as string;
    ...
    const completedUnitIds = new Set(completions.map((c: any) => c.unit_id).filter(Boolean));
...
router.get('/management/examination-summary', requirePermission('IDENTITY', 'IS_MANAGEMENT'), async (req: any, res: Response) => {
  try {
    const org_id = req.user.organization_id;
    const yearId = await AcademicContextResolver.resolveHistoricalAcademicYearId(req);
    ...
    const examStats = await prisma.studentExamResult.aggregate({
    ...
    const resultStatuses = await prisma.studentExamResult.groupBy({
      by: ['result_status'],
      where: { examination_id: latestExam.id },
      _count: { id: true }
    });
    ...
    for (const s of resultStatuses) {
      if (s.result_status === 'PASS') passCount += s._count.id;
      if (s.result_status === 'FAIL') failCount += s._count.id;
    }
...
router.get('/management/teacher-performance', requirePermission('IDENTITY', 'IS_MANAGEMENT'), async (req: any, res: Response) => {
  try {
    const org_id = req.user.organization_id;
    const yearId = await AcademicContextResolver.resolveHistoricalAcademicYearId(req);
...
router.get('/management/weak-subjects', requirePermission('IDENTITY', 'IS_MANAGEMENT'), async (req: any, res: Response) => {
  try {
    const org_id = req.user.organization_id;
    const yearId = await AcademicContextResolver.resolveHistoricalAcademicYearId(req);
    ...
    if (subjectStats.length === 0) return res.json([]);

    // Fetch subject names
    const subjects = await prisma.subject.findMany({
      where: { id: { in: subjectStats.map((s: any) => s.subject_id) } },
...
router.get('/management/student-risk', requirePermission('IDENTITY', 'IS_MANAGEMENT'), async (req: any, res: Response) => {
  try {
    const org_id = req.user.organization_id;
    const yearId = await AcademicContextResolver.resolveHistoricalAcademicYearId(req);
    ...
    if (config && config.config_data && typeof (config.config_data as any).threshold === 'number') {
      riskThreshold = (config.config_data as any).threshold;
    }
    ...
    const riskStudents = results.map((r: any) => {
...
router.get('/management/recent-activity', requirePermission('IDENTITY', 'IS_MANAGEMENT'), async (req: any, res: Response) => {
  try {
    const org_id = req.user.organization_id;
    ...
    for (const l of logs) {
      let type = 'OTHER';
      if (['EXAMINATION', 'STUDENT_EXAM_RESULT'].includes(l.entity_type)) type = 'EXAM_RESULT';
      else if (['SYLLABUS', 'UNIT', 'TOPIC', 'SUBTOPIC', 'COMPLETION_TRACKING'].includes(l.entity_type)) type = 'SYLLABUS_UPDATE';
      else if (['ACADEMIC_YEAR', 'GRADE', 'SECTION'].includes(l.entity_type)) type = 'ACADEMIC_YEAR';
      else if (['STUDENT_ENROLLMENT', 'TEACHER_ASSIGNMENT'].includes(l.entity_type)) type = 'ADMINISTRATIVE';
