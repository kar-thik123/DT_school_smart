...
import { getFlatPermissions, PERMISSION_DOMAINS } from '../config/permissions';
...
async function main() {
  ...
  const permissionMap: Record<string, string> = {}; // module:action -> id
    ...
    if (roleNameUpper === 'SUPER_ADMIN') {
      const rpData = Object.entries(permissionMap)
        .filter(([key]) => {
          ...
          return PERMISSION_DOMAINS[mod] === 'TENANT' && !(mod === 'IDENTITY' && act === 'IS_SYSTEM_ADMIN');
        })
        .map(([_, pid]) => ({
          role_id: role.id,
          permission_id: pid
