...
import { StudentMappingProcessor } from '../services/bulk-import/processors/student-mapping.processor';
import { StudentEnrollmentProcessor } from '../services/bulk-import/processors/student-enrollment.processor';
import { TeacherAssignmentProcessor } from '../services/bulk-import/processors/teacher-assignment.processor';
...
async function runTests() {
  ...
  if (!studentPerm) throw new Error('IDENTITY:IS_STUDENT not found');
  ...
  const mappingProcessor = new StudentMappingProcessor(org.id, 'test_user', 'dummy-year-id');
  ...
  const enrollmentProcessor = new StudentEnrollmentProcessor(org.id, 'test_user', 'dummy-year-id');
  ...
  const teacherProcessor = new TeacherAssignmentProcessor(org.id, 'test_user', 'dummy-year-id');
